...
num_shares = np.infty  # Number of times an agent will share their non-neutral belief; set np.infty for unlimited
...
def runModel(network, T):
    # create model
    model = KnowledgeModel(network=network, num_shares=num_shares)
...
plt.title("N = {N}, T = {T}, S = {S}, Num = {avg}, Shares = {shr}".format(N=N, T=T, S=S, avg=round(avg_num_fake, 1), shr=num_shares))
plt.legend(loc="lower center", ncol=3, fancybox=True, bbox_to_anchor=(0.5, 0.9))
plt.savefig("./output/N{N}-T{T}-S{S}-{shr}-{name}-{avg}.png".format(N=N, T=T, S=S, shr=num_shares, name=network.name,
